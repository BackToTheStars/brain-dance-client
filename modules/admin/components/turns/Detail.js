...
const YoutubeBlock = ({ turn, busy, setBusy, onRelocated, onError }) => {
  ...
  return (
    <div className="flex flex-col gap-2 border border-solid border-gray-300 rounded-md p-2">
      ...
      {/* Функция заморожена 22.08.2026: YouTube требует прохождения
          антибот-проверки и отвечает отказом уже на probe — и на проде, и
          локально. Кнопки намеренно оставлены (заморозка не удаление), плашка
          нужна, чтобы 502 не выглядел поломкой. Разбор и порядок проверок при
          разморозке — brain-platform/docs/youtube-relocate.md. */}
      <Alert
        type="warning"
        showIcon
        ...
        <>
          ...
          {!!formats.length && (
            <div className="flex flex-col gap-1">
              ...
              {/* Перенос медиа на свою статику идёт синхронно и долго,
                  прогресса нет: честно предупреждаем и не даём запустить
                  второй перенос. */}
